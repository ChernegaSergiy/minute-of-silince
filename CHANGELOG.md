...
### Added
- Add `glib` dependency for target Linux to enable setting the program name.
- Disable user viewport scaling in index.html to prevent zoom gesture scaling.

### Changed
- Update acknowledgments in the About tab to explicitly credit Bohdan Hdal's original idea and voice.

### Fixed
- Fix window grouping and application icon under pure Wayland in Snap environment by dynamically setting program name.


## [0.10.2] - 2026-05-31

### Added
- Personal remembrance dates management with name carousel on the visual overlay.

### Changed
- Migrate settings persistence to Tauri `plugin-store` and add dynamic settings change listener.
- Switch APNG progress ring rendering to `upng-js` for better compatibility and performance.
- Restructure frontend code into dedicated components and utils directories.
...
### Added
- New announcement voice: "Grinchenko University Radio" (Radio BG) — audio recording from Grinchenko University Radio.
...
- Copy logs flow shows clipboard success/failure feedback directly in the About tab, making the diagnostic action clearer for users.
